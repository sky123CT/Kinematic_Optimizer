...
class DirectCollocationT:
    ...
    def __define_obj_function__(self,
        ...
        obj = movement_cost + input_cost + time_cost # (distance_cost + orientation_cost + input_cost + time_cost)
        return obj
    ...
    def __define_constraints__(self,
        ...
        # dynamic constraints
        for t in range(1, self._time_interval_num+1):
            g += [self._q[:, t] - self._q[:, t - 1] - 1 / 2 * (self._q_dot[:, t] - self._q_dot[:, t - 1]) *
                        (self._t / self._time_interval_num)]
